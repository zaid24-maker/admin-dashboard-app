...
const Schedules = () => {
    ...
    const fetchSchedules = async () => {
        try {
            const res = await fetch(`\${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/schedules`, {
    ...
    const handleToggle = async (id) => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/schedules/${id}/toggle`, {
                method: 'PATCH',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            });
            const data = await res.json();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/schedules/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            });
