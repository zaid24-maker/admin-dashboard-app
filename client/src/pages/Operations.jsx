...
const Operations = () => {
    ...
    const fetchOperations = async () => {
        try {
            const res = await fetch('http://localhost:5001/api/executions/operations', {
    ...
    const handleEditTarget = async (id, currentTarget) => {
        ...
        try {
            const res = await fetch(`http://localhost:5001/api/users/${id}/target`, {
                method: 'PUT',
                headers: {
                        ...
                        return (
                            <motion.div key={user._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                                ...
                                <div className="flex space-x-4 items-center">
                                    {user.avatar ? (
                                        <img src={`http://localhost:5001${user.avatar}`} className="w-14 h-14 rounded-full object-cover border-2 border-slate-700 shadow-md" />
                                    ) : (
