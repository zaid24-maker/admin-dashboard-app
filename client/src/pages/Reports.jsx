    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
                <div>
                    <h1 className="text-3xl font-extrabold text-white mb-1">Execution Reports</h1>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <StatCard label="Total Runs" value={data.total} icon={BarChart2} color="indigo" />
                        <StatCard label="Successful" value={data.success} icon={CheckCircle2} color="emerald"
                            sub={data.total ? `${Math.round(data.success / data.total * 100)}% success rate` : '—'} />
                        <StatCard label="Failed" value={data.failed} icon={XCircle} color="rose" />
                        <StatCard label="Avg Duration" value={data.avgDuration ? `${(data.avgDuration / 1000).toFixed(1)}s` : '—'} icon={Clock} color="violet" />
                    </div>
